...
fun getFilesBySequentially(
    ...
    types: List<String> = listOf(*com.ntduc.baseproject.constant.FileType.ALL)
): List<BaseFile> {
...
fun Context.getImages(types: List<String> = listOf(*FileType.IMAGE)): List<BaseImage> {
    val images = ArrayList<BaseImage>()
    ...
    // Tạo điều kiện lọc để chỉ lấy các loại file được chỉ định
    var selection = ""
    for (i in types.indices) {
        if (i == 0) {
...
fun Context.getAllImages(): List<BaseImage> {
    val images = ArrayList<BaseImage>()
    ...
    // Không cần điều kiện lọc
    val sortOrder = "${MediaStore.Images.ImageColumns.DATE_ADDED} DESC"

    this.contentResolver.query(
        uri,
        projection,
        null, // Không áp dụng điều kiện lọc
        null,
        sortOrder
    )?.use { cursor ->
        val col_id = cursor.getColumnIndex(BaseColumns._ID)
